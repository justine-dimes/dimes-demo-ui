...
describe('scheduleStateAtPrice', () => {
  it('reports the quiet zone above the buffer floor', () => {
  ...
  it('accumulates fired steps at a mid-ladder price', () => {
  ...
  it('fires the debt-clear exit as soon as the sweep crosses its line, even inside the ladder', () => {
    ...
    expect(state.remainingFraction).toBeCloseTo(0.1428, 3)
    expect(state.nextEventKind).toBe('step')
    expect(state.nextEventPriceUsd).toBeCloseTo(0.2724, 10)
  })

  it('reports no next event below the whole plan, with the loan fully repaid', () => {
    const state = scheduleStateAtPrice(walkthrough(), 0.12)
    expect(state.firedStepCount).toBe(9)
    expect(state.debtClearFired).toBe(true)
    expect(state.loanRemainingUsd).toBe(0)
  ...
  it('names the debt-clear line as the next event when it is the highest unfired trigger', () => {
